...
data_SOL_parsed_comparison_registered <- data_SOL_parsed_comparison %>% filter(status_SOL != "geo")   

kable(table(data_SOL_parsed_comparison_registered$status_SOL,data_SOL_parsed_comparison_registered$newRegistrationUser_SOL)) %>% kable_styling()
...
merged_SOL_TMB_2 %>% group_by(test_name_TMB) %>% summarize(Frequency = n(), Proportion = round(n()/nCases,2)) %>% kable() %>% kable_styling()
